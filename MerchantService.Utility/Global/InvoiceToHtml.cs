   ...
   public class InvoiceToHtml
    {
        public static string get39(string s, double width, int height)
        {
            ...
            string result_html = ""; string color = ""; foreach (char c in result_bin)
            {
                ...
                result_html += "<div style=\"width:" + width + "px;height:" + height + "px;background:" + color + ";\"></div>";
            }
            result_html += "<div style=\"clear:both\"></div><br>";

            int len = ht['*'].ToString().Length;
            foreach (char c in s)
            {
                result_html += "<div style=\"width:" + (width * (len + 1)) + "px;color:#000000;text-align:center;\">" + c + "</div>";
            }
