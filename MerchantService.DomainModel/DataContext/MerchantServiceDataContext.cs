...
using MerchantService.DomainModel.Models.RewardPoint;

namespace MerchantService.DomainModel.DataContext
{
    public class MerchantServiceDataContext : IdentityDbContext<IdentityUser>
    {

        public MerchantServiceDataContext()
            : base("MerchantServiceDataContext", throwIfV1Schema: false)
        {
        ...
        public DbSet<AutomaticPODetail> AutomaticPODetail { get; set; }
        ...
        public DbSet<BalanceBarcodeConfiguration> BalanceBarcodeConfiguration { get; set; }

        public DbSet<BalanceBarcodeSection> BalanceBarcodeSection { get; set; }

        public DbSet<CompanyBarcodeConfiguration> CompanyBarcodeConfiguration { get; set; }

        public static MerchantServiceDataContext Create()
        {
            return new MerchantServiceDataContext();
        }
        ...
        {
            ChangeTracker.Entries().Where(x => x.Entity is MerchantServiceBase && x.State == EntityState.Added).ToList().ForEach(x =>
            {
                ((MerchantServiceBase)x.Entity).CreatedDateTime = DateTime.UtcNow;
            });
            ChangeTracker.Entries().Where(x => x.Entity is MerchantServiceBase && x.State == EntityState.Modified).ToList().ForEach(x =>
            {
                ((MerchantServiceBase)x.Entity).ModifiedDateTime = DateTime.UtcNow;
            });
